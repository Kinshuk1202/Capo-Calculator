...
class CapoToOpenViewModel:ViewModel(),Observable {
    ...
    var capoToOpenUtil:CapoToOpen

    init {
        selectedFret.value = "No Capo"
        enteredChord.value = ""
        finalchord.value = ""
        this.capoToOpenUtil = CapoToOpen()
    }
    ...
    fun updateNewChord(currentChord:String,fretNo:Int)
    {
        ...
        val curPositon = capoToOpenUtil.getCurrentScaleIdx(temp)
        setFretValue(fretNo)
        if(curPositon == -1)
        {
            finalchord.value = "-"
        ...
        else
        {
            ...
            val newScale = capoToOpenUtil.getNewScale(newPostion!!)
